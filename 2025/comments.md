...
Of course I tried to solve day 12 with a general algorithm. I flipped, rotated, and tried a lot. It did work on the test input, but it already took 5min tu run. So I figured, maybe I could narrow the candidates - not the case in the example, but that wouldn't be the first time the test is not solve the same way as the real input. It turns out, that was the only thing is expected, and I now have my last star ⭐
